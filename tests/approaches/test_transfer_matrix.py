...
def _create_obstruction_env_and_models(
    num_obstructions: int, render_mode: str | None = None
):
...
def test_transfer_matrix() -> None:
    ...
    print(f"Total test cases: {total_tests} learned tests (pure already cached)")
    print("=" * 70)
    ...
    for train_idx, train_complexity in enumerate(COMPLEXITY_LEVELS):
        for test_idx, test_complexity in enumerate(COMPLEXITY_LEVELS):
            ...
            print(
                f"Evaluating: Train on o{train_complexity} -> Test on o{test_complexity}"
            )
            ...
            print(f"Using {num_ngrams} n-grams from o{train_complexity} training")
