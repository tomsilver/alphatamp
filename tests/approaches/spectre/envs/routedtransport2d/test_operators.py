...
def test_eight_lifted_operators_with_expected_names() -> None:
    """Spec §8.3 #3: the operator vocabulary is exactly the eight names."""
...
def test_pick_top_then_place_side_blocked_by_grasp_precondition() -> None:
    ...
    # PickItemTop adds HeldGraspTop, not the HeldGraspSide that
    # PlaceItemSide's precondition requires.
...
def test_loaded_traverse_carries_item_param(color: str) -> None:
    """Loaded traverse signature is (robot, passage, src, dst, item)."""
...
def test_traverse_empty_signature() -> None:
    """Empty traverse signature is (robot, passage, src, dst) — no item."""
...
def test_passage_subtype_dispatch() -> None:
    """Each color maps to its passage subtype, all children of PassageType."""
