...
def _compute_filtered_prior_scores(
    applicable_indices: set[int],
    tried_failed: set[int],
    train_applicability: np.ndarray,
    train_success: np.ndarray,
) -> np.ndarray | None:
    """Return per-skeleton success rates from training rows matching the current state.

    Filters training rows where:
    - All skeletons in applicable_indices are applicable.
    - All skeletons in tried_failed are applicable and failed.

    Returns None if no training rows match (caller should fall back to global prior).
    """
    if not applicable_indices:
        return None

    applicable_cols = np.asarray(sorted(applicable_indices), dtype=np.int64)
    mask = train_applicability[:, applicable_cols].all(axis=1)

    if tried_failed:
        failed_cols = np.asarray(sorted(tried_failed), dtype=np.int64)
        mask = mask & train_applicability[:, failed_cols].all(axis=1)
        mask = mask & (train_success[:, failed_cols] == 0).all(axis=1)

    n_matching = int(mask.sum())
    if n_matching == 0:
        return None

    filtered_app = train_applicability[mask]
    filtered_suc = train_success[mask]
    app_counts = filtered_app.sum(axis=0)
    suc_counts = filtered_suc.sum(axis=0)
    rates = np.zeros(train_applicability.shape[1], dtype=np.float64)
    valid = app_counts > 0.5
    rates[valid] = suc_counts[valid] / app_counts[valid]
    return rates
...
def _run_filtered_prior_row(
    ...
    """Filtered-prior baseline: at each step, filter training rows by current applicable+failed
    state and pick the skeleton with the highest conditional success rate.

    Falls back to the global order when no training rows match the filter.
    fallback_counter is a mutable [total_decisions, fallback_decisions] list for diagnostics.
    """
    remaining = float(budget_seconds)
    elapsed = 0.0
    solved = False
    ...
    while True:
        untried_applicable = applicable_indices - tried
        if not untried_applicable:
            break

        fallback_counter[0] += 1
        scores = _compute_filtered_prior_scores(
            applicable_indices,
            tried_failed,
            train_data.applicability,
            train_data.success,
        )

        if scores is None:
            print(
                f"[filtered_prior] row {row_idx}: no matching training rows for "
                f"applicable={sorted(applicable_indices)} failed={sorted(tried_failed)}; "
                f"falling back to global prior"
            )
            fallback_counter[1] += 1
            # Fall back: pick next untried applicable skeleton from global order
            choice = next(
                (idx for idx in global_order.tolist() if idx in untried_applicable),
                None,
            )
        else:
            # Pick untried applicable skeleton with highest filtered score;
            # tie-break by global_order rank (lower rank = earlier in list = preferred)
            global_rank = {idx: rank for rank, idx in enumerate(global_order.tolist())}
            choice = max(
                untried_applicable,
                key=lambda idx: (float(scores[idx]), -global_rank.get(idx, len(global_order))),
            )

        if choice is None:
            break

        attempt_time = float(time_row[choice])
        if not _strict_attempt_allowed(attempt_time, remaining, epsilon):
            break
        ...
        if success_row[choice] > 0.5:
            solved = True
            break

        tried_failed.add(choice)
...
def _plot_success_curve(
    ...
    output_path: Path,
    dpi: int,
    filtered_prior_success: np.ndarray | None = None,
) -> None:
    ...
    if filtered_prior_success is not None:
        plt.plot(
            budgets,
            filtered_prior_success,
            label="Baseline (filtered-prior)",
            linewidth=2,
        )
...
def _plot_time_curve(
    ...
    output_path: Path,
    dpi: int,
    filtered_prior_values: np.ndarray | None = None,
) -> None:
    plt.figure(figsize=(8, 5))
    ...
    if filtered_prior_values is not None:
        plt.plot(
            budgets,
            filtered_prior_values,
            label="Baseline (filtered-prior)",
            linewidth=2,
        )
...
def main(cfg: DictConfig) -> None:
    ...
    filtered_prior_fallback_counter: list[int] = [0, 0]

    def filtered_prior_row_runner(
        row_idx: int,
        applicable_row: np.ndarray,
        success_row: np.ndarray,
        time_row: np.ndarray,
        ...
        return _run_filtered_prior_row(
            row_idx,
            applicable_row,
            success_row,
            time_row,
            ...
            epsilon,
            train_split,
            filtered_prior_fallback_counter,
        )
    ...
    filtered_prior_metrics = _evaluate_policy(
        "baseline_filtered_prior",
        filtered_prior_row_runner,
        test_split,
        budget_seconds,
    )
    total_fp = filtered_prior_fallback_counter[0]
    fallback_fp = filtered_prior_fallback_counter[1]
    print(
        f"[filtered_prior] fallback rate: {fallback_fp}/{total_fp} "
        f"({100 * fallback_fp / max(total_fp, 1):.1f}%)"
    )
    ...
    filtered_prior_time_success_only_curve = []
    ...
    filtered_prior_time_total_curve = []
    for sweep_budget in budgets.tolist():
        ...
        filtered_prior_sweep = _evaluate_policy(
            "baseline_filtered_prior",
            filtered_prior_row_runner,
            test_split,
            float(sweep_budget),
        ...
        filtered_prior_success_curve.append(float(filtered_prior_sweep["success_rate"]))
        ...
        filtered_prior_time_success_only_curve.append(
            float(filtered_prior_sweep["mean_time_success_only"])
        )
        ...
        filtered_prior_time_total_curve.append(float(filtered_prior_sweep["mean_time_total"]))
    ...
    filtered_prior_success_curve_np = np.asarray(filtered_prior_success_curve, dtype=np.float32)
    ...
    filtered_prior_time_success_only_curve_np = np.asarray(
        filtered_prior_time_success_only_curve,
        dtype=np.float32,
    )
    ...
    filtered_prior_time_total_curve_np = np.asarray(
        filtered_prior_time_total_curve,
        dtype=np.float32,
    )
    ...
    summary = {
        ...
        "baseline_filtered_prior": {
            "success_rate": float(filtered_prior_metrics["success_rate"]),
            "solved_count": int(filtered_prior_metrics["solved_count"]),
            "failed_count": int(filtered_prior_metrics["failed_count"]),
            "mean_time_success_only": float(filtered_prior_metrics["mean_time_success_only"]),
            "mean_time_total": float(filtered_prior_metrics["mean_time_total"]),
            "fallback_decisions": fallback_fp,
            "total_decisions": total_fp,
            "fallback_rate": float(fallback_fp / max(total_fp, 1)),
        },
        "encoder": {
    ...
    np.savez(
        ...
        filtered_prior_success_curve=filtered_prior_success_curve_np,
        ...
        filtered_prior_time_success_only_curve=filtered_prior_time_success_only_curve_np,
        ...
        filtered_prior_time_total_curve=filtered_prior_time_total_curve_np,
        ...
        filtered_prior_outcomes=filtered_prior_metrics["outcomes"].astype(np.int8),
        ...
        filtered_prior_elapsed_times=filtered_prior_metrics["elapsed_times"].astype(np.float32),
        ...
        filtered_prior_success_rate=np.float32(filtered_prior_metrics["success_rate"]),
        ...
        filtered_prior_mean_time_success_only=np.float32(
            filtered_prior_metrics["mean_time_success_only"]
        ),
        ...
        filtered_prior_mean_time_total=np.float32(filtered_prior_metrics["mean_time_total"]),
    ...
    _plot_success_curve(
        ...
        success_curve_path,
        dpi,
        filtered_prior_success=filtered_prior_success_curve_np,
    )
    _plot_time_curve(
        budgets,
        ...
        dpi=dpi,
        filtered_prior_values=filtered_prior_time_success_only_curve_np,
    )
    _plot_time_curve(
        budgets,
        ...
        dpi=dpi,
        filtered_prior_values=filtered_prior_time_total_curve_np,
    )
