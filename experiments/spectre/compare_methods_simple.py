...
def _(mo):
    mo.md(r"""# Method comparison (SIMPLE coverage/waste) — SPECTRE vs the low-level and zero-shot baselines

          The `compare_methods.py` experiments, but SPECTRE is trained with the earlier
          **simple/legacy coverage/waste** (`--legacy-coverage`:
          `coverage=|S∩culprits|/|culprits|`, `waste=|S\culprits|/|S|`) instead of the
          deployed unified definitions, and **`repeat` is turned on to carry SB2D** (the
          simple coverage/waste is inert there). Every baseline is grafted UNCHANGED from
          the deployed cache, so only the SPECTRE rows and the §4 `+scalars`/`full` arms
          differ. Held-out **test split**, **rollout FP** (lower is better). **Pick the
          environment below.** See `docs/decisions/07-stickbutton2d.md` 2026-08-27.
          """)
    return
...
def _(mo):
    ...
    # This is the SIMPLE-coverage/waste notebook, so the picker is restricted to the
    # simple env_variants (docs/decisions/07 2026-08-27). Falls back to the full registry
    # if neither is present, so the file still runs against a stock checkout.
    _simple_envs = [k for k in ("dd2d_simple", "sb2d_simple") if k in compare_envs.ENVS]
    _options = _simple_envs or list(compare_envs.ENVS)
    _default_env = os.environ.get("SPECTRE_COMPARE_ENV", _options[0])
    if _default_env not in _options:
        _options = [_default_env] + _options
