...
PRESETS: dict[str, dict[str, str]] = {
    ...
    # Learned-pathway workstream (docs/failed_records_fix.md F-B2). The deployed direction is
    # the pre-pooling **StepJoin** (`--step-join`, the C2 fix — candidate step tokens join over
    # the evidence memory before pooling), on the SAME ablation backbone as `abl_only_records`
    # (the rung-0 tokens-only baseline, retrained here as `fr_summary`). Result 2026-08-22:
    # `fr_join` −1.80 [−3.52,−0.16] (23%→43% gap-closure). **C1 content enrichment
    # (`--record-mode steps`) was CUT** — inert alone (`fr_steps` −0.04) and harmful combined
    # (`fr_steps_join` +0.73, attention dilution); the machinery stays flag-gated (off =
    # byte-identical) per the build-then-disable convention but is not swept. Run per env one
    # seed at a time (restock adds ` --scene-3d`):  --preset failed_records --env dd2d_v4 --seeds 0
    "failed_records": {
        "fr_summary": (
