...
4. **Train** (the deployed recipe; multi-seed):
   **Deployed = a two-stage RESIDUAL-ADAPTIVE procedure (since 2026-08-27; driver `experiments/spectre/refresh_dd2d_sb2d_train.sh`).** (1) Train the frozen pure-geometry trunk: `train … --no-overlap --no-records <backbone> --out-suffix _abl_static` (backbone = `--use-pca-feats --use-edgeconv --use-point-sab --pma-seeds 4 --atom-mode profiles --select-window 5`). (2) Train the residual on top: `train … <backbone> --overlap-mode jaccard --coverage-feats --repeat-feats --aggregate-records --evidence-attn --state-delta --residual-adaptive --freeze-static --init-static-from data/spectre/checkpoints_spectre_norec_noov_atoms_abl_static/dd2d_v4/seed_{seed}/best.pt --out-suffix _resid_full` → `checkpoints_spectre_atoms_resid_full`.
   — `ResidualEvidenceScorer` freezes the trunk and routes ALL failure-conditioned features (records + compiled scalars) into a zero-init |F|-gated residual (`logit = static + g(|F|)·adjustment`). This **fixed the jointly-trained `+records` inertness** (W3 interference): DD2D `+records` net-negative **+1.33 (joint) → net-positive −0.48 (residual)**. **Deployed FP: DD2D adaptive 6.42, SB2D 1.98** (ADR/notebook `decisions/07`/`notebook/07` 2026-08-27). **`--step-join` is dropped** (it perturbs the frozen static input); `--repeat-feats` is INERT on DD2D/SB2D (graceful degradation 2026-08-26; load-bearing only on restock3d). §4 is a 4-arm residual decomposition (static / +records / +scalars / full); **scalars carry the bulk (DD2D −11.72), records are a modest net-positive residual**. Per-seed breakdown tables (§1b/§2b.1/§4.3b) sit beside every multi-seed figure.
   — The **superseded joint baseline** (single-stage, `--step-join`, no residual; `checkpoints_spectre_atoms_v3final`, DD2D 7.11 / SB2D 1.88, `sweep --preset v3final`) is frozen in `docs/joint_refresh_snapshot.md`:
   `python -m alphatamp.approaches.spectre.train --env dd2d_v4 --seed 0 --overlap-mode jaccard --coverage-feats --repeat-feats --aggregate-records --evidence-attn --state-delta --select-window 5 --use-pca-feats --use-edgeconv --use-point-sab --pma-seeds 4 --atom-mode profiles --step-join`
