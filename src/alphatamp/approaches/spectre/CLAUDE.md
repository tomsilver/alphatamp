...
| **Restock3D** — the third evaluation environment (3D / MuJoCo TidyBot) | `src/alphatamp/approaches/spectre/envs/restock3d/` — a MuJoCo restock task built on the `envs/shelf3d/` scaffolding: store floor cubes + tall blocks into single-object shelf regions across a **short cell** and a **tall cell** (Config B). `models.py` (domain: `Pick`/`Place(obj,region)`, `InRegion`/`Stored`, **no `Clear`** so capacity/height are invisible), `geometry.py` (`place_gate`: **F2** self-inflicted over-assignment + **F3** height mismatch), `refine.py` (symbolic feasibility walk — the label/evidence source), `instrumented_refiner.py` (physics pick + deterministic geometric place, for the demo), `generator.py` (strata r0-r3 by `d=(σ_tall,σ_short)`), `strata.py`, `region_geometry.py`. Env_variant **`restock3d_v1`**. Entry points `experiments/spectre/restock3d_{collect,difficulty,demos}.py`; `conf/env/restock3d_v1.yaml`. **Feasibility is geometry-gated (DD2D lesson), not physics** — the honest MuJoCo-direct realization that avoids ShelfObstruct3D's inertness. v1 leads with F2+F3; **F1 (grasp obstruction) + coverage/waste + learned baselines are deferred.** ADR/notebook: `decisions/07` / `notebook/07` 2026-08-14 |
