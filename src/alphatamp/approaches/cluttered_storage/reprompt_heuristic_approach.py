"""
Hybrid approach: LLM generates a blind plan first (reprompt style),
then on failure feeds the failure context to generate heuristics (heuristic style).
"""
...
from alphatamp.approaches.cluttered_storage.reprompt_approach import (
    RepromptLLMAbstractPlanGenerator,
)
...
class HeuristicGenerator(RelationalHeuristicSearchAbstractPlanGenerator):
    """A generator that uses an LLM-generated heuristic instead of hFF."""
    ...
    def _relational_heuristic_factory(
        ...
        pyperplan_heuristic = create_pyperplan_heuristic_from_fn(
            self._generate_heuristic_fn, self._pddl_domain, pddl_problem, ground_operators
        )
...
class RepromptHeuristicApproach(BaseApproach[_O, _X, _U]):
    """
    LLM first attempts a blind plan (reprompt style).
    On failure, feeds the failure context to generate heuristics for A* planning.
    """
    ...
    def __init__(
        ...
        initial_plan_timeout: float = 50.0,
    ):
        ...
        self._initial_plan_timeout = initial_plan_timeout
    ...
    def _make_heuristic_generator(self, generate_heuristic_fn: Callable) -> HeuristicGenerator:
    ...
    def _make_planner(self, generator: Any) -> SesamePlanner:
    ...
    def _evaluate_heuristic(
        ...
        generator = self._make_heuristic_generator(generate_heuristic_fn)
    ...
    def _build_failure_section(failure_context: Dict[str, Any]) -> str:
        ...
        return (
            ...
            "-------------------------\n"
            "An LLM-generated plan was attempted but failed during low-level trajectory "
            "sampling. Use this failure information to generate heuristics that can "
            "guide a search planner past this failure point.\n"
    ...
    def _run_planning(
        ...
        # ── Step 1: LLM generates a blind plan (reprompt approach style) ─────
        print("\n=== Step 1: LLM generating blind plan ===")
        ...
        plan, _ = initial_planner.run(
            problem, timeout=min(self._initial_plan_timeout, timeout)
        )
        self.last_metrics = tracking_refiner.metrics

        if plan is not None:
            print("Initial LLM plan succeeded.")
            return plan

        print("Initial LLM plan failed — switching to heuristic search.")

        remaining_time = timeout - (time.perf_counter() - start_time)
        if remaining_time <= 0:
            raise TimeoutError("No plan found")

        # ── Step 2: Extract failure context from the blind plan attempt ───────
        abstract_actions = initial_plan_generator._last_abstract_actions  # pylint: disable=protected-access
        abstract_states = initial_plan_generator._last_abstract_states  # pylint: disable=protected-access
        ...
        base_prompt = build_heuristic_prompt(initial_atoms, goal_atoms)
        ...
        if (failed_state is not None and abstract_actions
            ...
            heuristic_prompt = base_prompt + self._build_failure_section(failure_context)
        else:
            print("  No failure context available — generating heuristics without it.")
            heuristic_prompt = base_prompt

        # ── Step 3: Generate heuristics informed by failure context ───────────
        print(f"\n=== Step 3: Generating {self._num_candidates} heuristics with failure context ===")
        candidates = self._synthesize_all_heuristics(heuristic_prompt)
        if not candidates:
            raise RuntimeError("LLM returned no valid heuristic code blocks")

        # ── Step 4: Evaluate candidates, pick best ────────────────────────────
        print(f"\n=== Step 4: Evaluating candidates ({self._eval_timeout}s each) ===")
        best_fn, best_code, best_score = self._select_best_heuristic(
            problem, candidates, start_time, timeout
        )
        print(f"\n=== Best score: {best_score} — running full A* planning ===")

        # ── Step 5: Full A* planning with the best heuristic ─────────────────
        remaining_time = timeout - (time.perf_counter() - start_time)
        generator = self._make_heuristic_generator(best_fn)
        planner = self._make_planner(generator)
