...
from alphatamp.approaches.practice_makes_perfect.settings import CFG
from alphatamp.structs import Array


class SkillCompetenceModel(abc.ABC):
    """A model that tracks and predicts competence for a single skill based on the
    history of outcomes and re-learning cycles."""
    ...
    def predict_competence(self, num_additional_data: int) -> float:
        """Predict what the competence for the next cycle would be if we were to collect
        num_additional_data outcomes during this cycle."""
...
class LegacySkillCompetenceModel(SkillCompetenceModel):
    ...
    def predict_competence(self, num_additional_data: int) -> float:
        ...
        return min(
            1.0, current_competence + CFG.skill_competence_initial_prediction_bonus
        )
...
class LatentVariableSkillCompetenceModel(SkillCompetenceModel):
    ...
    def _run_expectation_maximization(self) -> None:
        ...
        self._posterior_competences.append(self._competence_regressor.predict_beta(n))
    ...
    def _get_regressor_inputs(self) -> Array:
        ...
        num_data_before_cycle = np.array(
            [0] + num_data_after_cycle[:-1], dtype=np.float32
        )
